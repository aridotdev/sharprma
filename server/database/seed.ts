...
async function main() {
    ...
    const users = [
        { name: 'Admin User', userAuthId: 'admin@sharp.com', role: 'ADMIN' as const },
        { name: 'QRCC User', userAuthId: 'qrcc@sharp.com', role: 'QRCC' as const },
        { name: 'CS User', userAuthId: 'cs@sharp.com', role: 'CS' as const },
        { name: 'Management User', userAuthId: 'mgmt@sharp.com', role: 'MANAGEMENT' as const }
    ]

    for (const u of users) {
        await db.insert(schema.user)
            .values({ ...u, isActive: true, createdAt: new Date().toISOString() })
            .onConflictDoUpdate({
                target: schema.user.userAuthId,
                set: { name: u.name, role: u.role }
            })
            .run()
    }

    // 3. Seed Product Models (Samples)
